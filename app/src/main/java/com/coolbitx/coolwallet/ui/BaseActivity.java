...
//        final byte cwHdwAllAccountInfo = 0x05;
    ...
    public void SetCurrencyRate(Context mContext) {
        double mDbRate = DatabaseHelper.queryCurrent(mContext, AppPrefrence.getCurrentCountry(mContext));
        int currRate = (int) (mDbRate * 100);

        byte[] BigcurrData = ByteBuffer.allocate(4).putInt(currRate).order(ByteOrder.BIG_ENDIAN).array();

        byte[] currData = new byte[5];
        currData[0] = 0;
        System.arraycopy(BigcurrData, 0, currData, 1, 4);

        cmdManager.SetCurrencyRate(currData, new CmdResultCallback() {
                    @Override
                    public void onSuccess(int status, byte[] outputData) {
                        if ((status + 65536) == 0x9000) {
                            LogUtil.d("Set CurrencyRate onSuccess.");
                        }
                    }
                }
        );
    ...
    public class MyRunnable implements Runnable {
        ...
        public void run() {
            String result = "";
            try {
                result = cwBtcNetWork.doGet(cv, extraUrl, "");
                if (result != null) {
                    if (extraUrl.contains(BtcUrl.RECOMMENDED_TRANSACTION_FEES)) {
                        //feesRate
